...
DO_PDFLATEX_RAW = $(TEX_INTERPRETER) -shell-escape -interaction=nonstopmode fund >err
...
preflight:
	@perl -e 'if (-d "scripts/custom") {system("chmod +x scripts/custom/*"); foreach $$f(<scripts/custom/*>) {system($$f)}}'
...
prepress:
	make preflight_figs
	pdftk fund.pdf cat 3-end output fund_lulu.pdf
